...
function QuakeConsole:display()
   ...
   local x, y
   local off = self.offset and geom.width * self.offset or 0
   if     self.horiz == "left"  then x = geom.x + off
   elseif self.horiz == "right" then x = geom.width + geom.x - width - off
...
function QuakeConsole:new(config)
   ...
   config.screen   = config.screen or awful.screen.focused()
